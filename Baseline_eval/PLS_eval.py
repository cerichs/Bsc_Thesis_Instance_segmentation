...
def PLS_class(classifier,images,img_path):
    ...
    for image, path in zip(images,img_path):
        ...
        rgb_image, img_tres = preprocess_image(fixed_path) # Uncomment to evaluate with watershed (ie. use predicted masks)
    ...
    for i in range(n):
        ...
        for j, anno in enumerate(dataset["annotations"]):
            if anno["image_id"] == i:
                ...
                label += 1
        cv.imwrite(f"I:/HSI/masks/{dataset['images'][i]['file_name'][:-4]}.png",empty_mask)
