        ...
        if (class_check[ids[0]]<= (n//len(class_list))):
            ...
            if keep:
                ...
                if not image_path.endswith(".npy"):
                    image_path = image_path[:-3]+"npy"
                
                img = np.load(image_path)
                #img = cv.cvtColor(img, cv.COLOR_BGR2RGB)
                
                
                subwindow_size = (256, 256, 102)
                ...
                np.save(f"I:/HSI/test_img/window{c}.npy",subwindow)
                c += 1 
    export_json(new_annotation,"I:/HSI/test_img/COCO_HSI_windowed.json")
